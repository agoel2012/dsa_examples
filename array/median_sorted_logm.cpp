...
double findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2) {
  // Brute force is to create a new array, sort and merged[middle] or (merged[lower_mid] + merged[upper_mid])/2 is median
  // To achieve log (M + N) time complexity, need to create a pivot element on each of array and track L, R of pivot on each and do binary search
  ...
  while (left <= right) {
	  ...
	  // Edge case: use INT_MIN, INT_MAX for cases when mid are either end of the array:
